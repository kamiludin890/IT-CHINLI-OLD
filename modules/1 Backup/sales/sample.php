...
//	echo "par:".	$id;
	...
	<li><label for='12'>".l(mysql_field_name($row, 12))."</label><textarea name='12' rows=6 cols=50 >$r[12]</textarea></li>
...
function cetak(){extract($GLOBALS);
	...
	$document = file_get_contents("prints/tmp-sample.rtf");
	...
	$fr = fopen('prints/sample.rtf', 'w') ;
	fwrite($fr, $document);
	fclose($fr);
 	echo "<script type='text/javascript'>window.open('prints/sample.rtf')</script>";
