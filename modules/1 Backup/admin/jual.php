...
function editform($id,$btn){ extract($GLOBALS); 
	...
	<li><label for='10'>".l(mysql_field_name($row, 10))."</label><input class='text'  name='10'  value='$r[10]' type=hidden />$r[10] xxxx</li>
...
//$_SESSION['idinduk']=$id; $_SESSION['idkontak']=$r[8]; 
//echo "<script type='text/javascript'>window.location.href='?menu=home&mod=pos/jual_items&res=h'<script>";
 	
 	$ids=getrow('id','delivery_invoice',"where invf1='$r[6]'");
	$query = "Update pos_transaksi set 
	subtotal=(SELECT sum(amount)  FROM delivery_invoice_items where induk='$ids[0]'),
	ppn=subtotal*0.1,
	total=ppn+subtotal
	where id='$id' ";	
	$result = mysql_query($query) or die('Error Album Select : '.$query);
	...
	<li class='buttons'><button type=submit value=$btn name='mybutton' class='formbutton' >".l($btn)."</button></li>
			...
			$ids=getrow('id','delivery_invoice',"where invf1='$r[6]'");
	 	$limit = 25;
 	if ($rest==''){$rest=" WHERE induk=$ids[0]";} else {$rest.=" AND induk=$ids[0]";}
	

	$tbl2='delivery_invoice_items';
	$fld2='id,kodebarang,namabarang,amount';

	
	table($tbl2,$fld2,$limit,$rest,$mod);
	$_SESSION['myquery']="SELECT $fld from $tbl $rest ";
	}
