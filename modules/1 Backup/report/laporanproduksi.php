<?php global  $title, $mod, $tbl, $fld, $akses ;
	$mod='report/laporanproduksi';
	$tbl='report_laporanproduksi';
	
	$fld='id,kode,tanggal,';
	for ($i=1;$i<=8; ++$i){$items[]="lpf".$i ; };
	$fld.=implode(",", $items);
...
//	echo "par:".	$id;
	...
	<li><label for='4'>".l(mysql_field_name($row, 4))."</label><input class='text' name='4' value='$r[4]' id='a' /></li>
	<li><label for='5'>".l(mysql_field_name($row, 5))."</label><input class='text' name='5' value='$r[5]' id='b'/></li>
	<li><label for='6'>".l(mysql_field_name($row, 6))."</label><input class='text' name='6' value='$r[6]' id='c'/></li>
	<li><label for='7'>".l(mysql_field_name($row, 7))."</label><input class='text' name='7' value='$r[7]' /></li>
	<li><label for='8'>".l(mysql_field_name($row, 8))."</label><input class='text' name='8' value='$r[8]' id='t'/></li>
	...
	function(){
		...
		function calculate(){
			t.innerHTML = t.value = a.value == 0 || !a.value? 0 : a.value == 1? t.value : parseInt(a.value) + parseInt(b.value) + parseInt(c.value);
		}
		calculate();
		a.onkeyup = a.onmouseout = calculate;
		b.onkeyup = b.onmouseout = calculate;
		c.onkeyup = c.onmouseout = calculate;
	}
	)();
	</script> ";
