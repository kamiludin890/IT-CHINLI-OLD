...
//JUDUL
echo "<center><h1>Data Komputer PT Chinli Plastic Technology Indonesia</h1></center>";
...
if($_POST['upload']){
	...
	if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
		if($ukuran < 1044070){
			...
			$query = mysql_query("INSERT INTO datait_komputer SET foto_cpu='$nama',nama_komputer='$nama_komputer',tanggal_pembelian='$tanggal_pembelian',pengguna='$pengguna',status='$status',kondisi='$kondisi',departement='$departement',bagian='$bagian'");
			if($query){
...
$delete_database="DELETE FROM datait_komputer WHERE foto_cpu='$foto_cpu'";
	...
	if($_POST['save']){
		...
		if ($nama1=='') {$query_update="";}else{$query_update="foto_cpu='$nama',"; move_uploaded_file($file_tmp, 'modules/datait/file/'.$nama);
			...
			unlink($target);}//DELETE NOW
				...
				$query = mysql_query("UPDATE datait_komputer SET $query_update nama_komputer='$nama_komputer',tanggal_pembelian='$tanggal_pembelian',pengguna='$pengguna',status='$status',kondisi='$kondisi',departement='$departement',bagian='$bagian'");
}}
...
//JUDUL BAGIAN
if ($bagian==''){}else{echo "<center><h3>Bagian $bagian</h3></center>";}

//AWAL PILIHAN DEPARTEMENT
echo "<form method ='post' action='?menu=home&mod=datait/komputer'>
<table>
<tr>
 <td>Departement</td>
 <td>:</td>
...
echo "<table><form action='?menu=home&mod=datait/komputer' method='post' enctype='multipart/form-data'>
			<tr>
			<td>FOTO CPU</td>
			<td>:</td>
...
elseif($rincian=='rincian') {
	...
	$sql2="SELECT * FROM datait_komputer WHERE id='$id_items'";
	...
	echo "<table><form action='?menu=home&mod=datait/komputer' method='post' enctype='multipart/form-data'>
				...
				<form action='?menu=home&mod=datait/komputer' method='POST'>
				<input type='hidden' name='departement' value='$rows2[departement]'/>
				<input type='hidden' name='bagian' value='$rows2[bagian]'/>
				...
				<td><input type='submit' style='width:100%;' value='Tambah Sparepart'></td></tr>
				</form></br>";

				//Tambah Hapus Update Sparepart
				if ($tambah_sparepart=='tambah_sparepart') {
				$insert_sparepart="INSERT INTO datait_komputer_items SET induk='$id_items'";
				mysql_query($insert_sparepart);
				}

				if ($update_sparepart=='update_sparepart') {
				$update_sparepart="UPDATE datait_komputer_items SET merek='$merek',keterangan_sparepart='$keterangan_sparepart',status_sparepart='$status_sparepart',nama_sparepart='$nama_sparepart',type='$type',size='$size',no_unik='$no_unik',tanggal_pembelian_sparepart='$tanggal_pembelian_sparepart',kondisi_sparepart='$kondisi_sparepart' WHERE id='$id_sparepart'";
				mysql_query($update_sparepart);

				$update_sparepart_history="INSERT INTO datait_komputer_history SET merek='$merek',keterangan_sparepart='$keterangan_sparepart',status_sparepart='$status_sparepart',nama_sparepart='$nama_sparepart',type='$type',size='$size',no_unik='$no_unik',tanggal_pembelian_sparepart='$tanggal_pembelian_sparepart',kondisi_sparepart='$kondisi_sparepart',induk='$id_items'";
				mysql_query($update_sparepart_history);
				}

				if ($delete_sparepart=='delete_sparepart') {
				$delete_sparepart="DELETE FROM datait_komputer_items WHERE id='$id_sparepart'";
				...
				<th align=center width=1%>Tanggal Pembelian Sparepart</th>
				...
				$sql3="SELECT * FROM datait_komputer_items WHERE induk='$id_items' ORDER BY id";
				$result3=mysql_query($sql3);
				$n=1;
				while ($rows3=mysql_fetch_array($result3)) {
				echo "<form action='?menu=home&mod=datait/komputer' method='POST'>
				<tr>
				<td style='padding: 5px; color:black; align='center' align='center' width='10px' bgcolor='#FFFFFF'><strong>$n</strong></td>

				<td><select name='nama_sparepart' style='width:100px align:center'>
							<option value='$rows3[nama_sparepart]'>$rows3[nama_sparepart]</option>
							<option value='Motherboard'>Motherboard</option>
							<option value='Proccesor'>Proccesor</option>
							<option value='Kipas Processor'>Kipas Processor</option>
			...
			<td>
				...
				<table align='center' width='100%' border='' cellpadding='' cellspacing='' bgcolor='#F0FFFF'>
					<tr>
						...
						<td style='padding: 5px; color:black; background-color: #F0FFFF;' align='center' width='auto' bgcolor='#FFFFFF'><strong>Foto CPU</strong></td>
						<td style='padding: 5px; color:black; background-color: #F0FFFF;' align='center' width='auto' bgcolor='#FFFFFF'><strong>Nama Komputer</strong></td>
						...
						<td style='padding: 5px; color:black; background-color: #F0FFFF;' align='center' width='auto' bgcolor='#FFFFFF'><strong>Pengguna</strong></td>
...
//ISI TABEL
$sql1="SELECT * FROM datait_komputer WHERE departement='$departement' AND bagian='$bagian'";
...
if ($rincian=='rincian') {
echo "<table></br>
<h2 align='center'>History Komputer</h2>
<table class=table width=100% align=center border=1>
<tr style='background-color:#C0C0C0;'>
<th align=center width=1%>No</th>
<th align=center width=7%>Nama Sparepart</th>
...
//ISI TABEL HISTORY
$sql4="SELECT * FROM datait_komputer_history WHERE induk='$id_items' ORDER BY id";
