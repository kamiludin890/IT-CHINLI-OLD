...
while ($rows=mysql_fetch_array($result)){
	...
	$urutan_item=$r;
		...
		//TOTAL QTY MASUK
		$result14=mysql_query("SELECT SUM(qty) as tqty FROM admin_purchasing_dokumen_masuk WHERE induk='$rows[id]' AND kodebarang='$rows1[kode_barang]'");
...
header("Content-Disposition: attachment;filename=balance_order.xls");
