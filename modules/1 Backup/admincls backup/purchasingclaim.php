...
$address='?mod=admincls/purchasingclaim';
...
if ($_POST['id_distribusi_masuk']){
	...
	if (ambil_database(id_distribusi_masuk,admin_claim_purchasing_dokumen_masuk,"id_distribusi_masuk='$id_distribusi_masuk'")!=$id_distribusi_masuk) {
		mysql_query("INSERT INTO admin_claim_purchasing_dokumen_masuk SET induk='$id',nomor_doc='$nomor_doc',tanggal_doc='$tanggal_doc',supplier='$supplier',id_distribusi_masuk='$id_distribusi_masuk'");
	}
}//INSERT DOKUMEN KIRIM END

	//INSERT DOKUMEN PO
	if ($_POST['id_asal_po']){
		mysql_query("UPDATE $nama_database SET id_asal_po='$_POST[id_asal_po]' WHERE id='$id'");
	}
	//INSERT DOKUMEN PO END
...
$ambil_id=ambil_database(id,admin_claim_purchasing_items,$data2);
$ambil_usd1=ambil_database(price_usd,admin_claim_purchasing_items,"id='$ambil_id'");
$ambil_rp1=ambil_database(price_rp,admin_claim_purchasing_items,"id='$ambil_id'");
$ambil_nt1=ambil_database(price_nt,admin_claim_purchasing_items,"id='$ambil_id'");
$ambil_jumlah_spk=ambil_database(qty_purchasing,admin_claim_purchasing_items,"id='$ambil_id'");
...
if (base64_decrypt($_GET['hapus_d'],"XblImpl1!A@")) {
	...
	$delete="DELETE FROM admin_claim_purchasing_dokumen_masuk WHERE id='$id_items'";
	mysql_query($delete);
}
...
echo "<td>";
      // <form method ='POST' action='modules/admincls/cetak/cetak_claim_purchasing.php' target='_blank'>
...
echo "</td><td>
			<form method ='POST' action='modules/admincls/cetak/print_excel_purchasing_claim.php' target='_blank'>
...
echo "<tr>";
echo "<td><center><img src='modules/gambar/logo_lengkap.png' width='50%' height='10%'/><center></td>";
echo "</tr>";echo "<tr>";
...
echo "</tr>";
echo "<tr>";echo "<td></td>";echo "<td></td>";echo "<td></td>";
echo "<td><strong>".ambil_database($bahasa,pusat_bahasa,"kode='dokumen_asal_po'")."</strong></td>";

echo "</td>";
echo "<form method='POST' action='$address'>";
//$nilai_id_distribusi_masuk=ambil_variabel_kutip_satu(admin_claim_purchasing_dokumen_masuk,id_distribusi_masuk);
$kepada=ambil_database(kepada,$nama_database,"id='$id'");
$sql01="SELECT * FROM admin_purchasing WHERE kepada='$kepada'"; //WHERE status='Selesai' AND kegiatan='masuk' AND jenis_doc LIKE 'BC %' AND selesai_masuk NOT LIKE '0000-00-00' AND id NOT IN ($nilai_id_distribusi_masuk) AND tanggal > '2020-12-01' ORDER BY tanggal
$result01=mysql_query($sql01);

$id_asal_po=ambil_database(id_asal_po,$nama_database,"id='$id'");
$result04=mysql_query("SELECT * FROM admin_purchasing WHERE id='$id_asal_po'");
$rows04=mysql_fetch_array($result04);
echo "<td>
<select class='comboyuk' name='id_asal_po'>
<option value='$id_asal_po'>$rows04[tanggal] | $rows04[po_purchasing] | $rows04[jenis_po_purchasing] | $rows04[kepada]</option>";
while ($rows01=mysql_fetch_array($result01)){
echo "<option value='$rows01[id]'>$rows01[tanggal] | $rows01[po_purchasing] | $rows01[jenis_po_purchasing] | $rows01[kepada]</option>";}
echo "</select>
</td>";
...
echo "<a target='_blank' href='?mod=admincls/purchasing&id=".base64_encrypt($id_asal_po,"XblImpl1!A@")."&opsi=".base64_encrypt("item","XblImpl1!A@")."&halaman=$nomor_halaman&pilihan_tahun=$pilihan_tahun&pilihan_bulan=$pilihan_bulan&pilihan_pencarian=$pilihan_pencarian&pencarian=$pencarian'><img src='modules/gambar/see.png' width='30px'/></a>";
echo "</td>";
...
$sql02="SELECT * FROM admin_claim_purchasing_dokumen_masuk WHERE induk='$id'";
...
echo '<a href="#" onClick="window.open(\''."modules/warehouse/gambarproduk/tampil_foto.php?gambar=$nama_gambar_tampilan".'\', \''.'mywindow'.'\', \''.'status=1,toolbar=1'.'\')">'."<img src='modules/warehouse/gambarproduk/$nama_gambar_tampilan' width='80' height='100'/>".'</a>';
echo "</td>";
//FOTO END

//TOTAL MASUK PER DOKUMEN
	$result15=mysql_query("SELECT SUM(qty) as tqty FROM admin_claim_purchasing_dokumen_masuk WHERE induk='$id' AND kodebarang='$rows4[kode_barang]'");
...
echo "<td style='text-align:left; padding-left:10px; white-space:nowrap;'>";
	$result16=mysql_query("SELECT * FROM admin_claim_purchasing_dokumen_masuk WHERE induk='$id' AND kodebarang='$rows4[kode_barang]'");
