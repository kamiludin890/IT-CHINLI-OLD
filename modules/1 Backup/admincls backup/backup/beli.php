...
function home(){extract($GLOBALS);	global  $result;
	$fld='id,kode,tanggal,pembuat,lokasi,keterangan,status,kontak,subtotal,ppn,total' ;
 	if ($rest==''){$rest=" where kegiatan='beli'";} else { $rest.=" and kegiatan='beli'";}
	$limit = 50;
	$result='home' ;
...
function editform($id,$btn){ extract($GLOBALS); 
	...
	<li><label for='8'>".l(mysql_field_name($row, 8))."</label>". droprow('8','id,nama','pos_kontak',$r[8],"where kategori='supplier'" )."</li>
	...
	echo "<script type='text/javascript'>window.location.href='?menu=home&mod=admincls/beli_items&res=h'</script>";
	}
 	}	
...
function lanjut(){ extract($GLOBALS);
	...
	$id=mysql_insert_id();
	$_SESSION['idinduk']=$id; 
	echo "<script type='text/javascript'>window.location.href='?menu=home&mod=admincls/beli_items'</script>";
  	}	
