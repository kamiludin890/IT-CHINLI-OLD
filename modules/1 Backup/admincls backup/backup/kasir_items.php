...
function editmenu(){extract($GLOBALS);	
	...
	if($jual['status']=='Selesai'){$btn='struk'; echo usermenu($btn);} else {echo usermenu('selesai,barang,delete');}}
	}
...
function home(){extract($GLOBALS);global  $result;
	...
	<li><label for='11'>Dibayar</label><input class='text'  value='$r[14]' name=dibayar id='dibayar'  /></li>
	<li><label for='11'>Kembali</label><input class='text'  value='$r[15]' name=kembali id='kembali'  /></li>
	<button type=submit value='bayar'  name='mybutton' class='formbutton' $s >".l('bayar')."</button></li>
	...
	window.open('display.php?d=DIBAYAR&v='+dibayar.value,'mahfut','width=600,height=300,location=no,scrollbars=no,status=no');
 	}
	function Func1(){ window.open('display.php?d=KEMBALI&v='+kembali.value,'mahfut','width=600,height=300,location=no,scrollbars=no,status=no');}

	calculate();
	dibayar.onkeyup = dibayar.onmouseout = calculate;
	kembali.onkeyup = kembali.onmouseout = Func1;
...
function bayar(){ extract($GLOBALS);  

	$induk=$_SESSION['idinduk'];

	$query="UPDATE pos_transaksi SET dibayar='$_POST[dibayar]', kembali='$_POST[kembali]' WHERE id='$induk'";
	mysql_query($query)or die('Error 1 '.$query); 
...
$total=getrow("*","pos_transaksi"," where id='$induk'");
echo "<script type='text/javascript' language='JavaScript'>
window.open('display.php?d=$barang[nama]&v=$barang[hargajual]','mahfut','width=600,height=300,location=no,scrollbars=no,status=no');
function Func1(){ window.open('display.php?d=TOTAL&v=$total[total]','mahfut','width=600,height=300,location=no,scrollbars=no,status=no');}
...
$query1="Select nama,banyak,hargajual,jual from pos_transaksi_items WHERE induk='$induk'";
...
$Data .= $setting['alamat']. "\n";
$Data .= "-------------------------------------------\n";
$Data .= "Tanggal". $tab .":". $row['tanggal']  ."\n";
$Data .= "Kasir  ". $tab .":". $row['pembuat']  ."\n";
$Data .= "Faktur ". $tab .":". $row['kode']  ."\n";
$Data .= "-------------------------------------------\n";

while ($row1=mysql_fetch_array($result1)):
$banyak=ceil($row1['banyak']);
$hargajual=format_rupiah($row1['hargajual']);
$jual=format_rupiah($row1['jual']);

$Data .= $row1['nama']."\n";
$Data .= $banyak." x ".$tab.$hargajual.$tab."=".$jual."\n";
endwhile;
$Data .= "\n";
$subtotal=format_rupiah($row['subtotal']);
$ppn=format_rupiah($row['ppn']);
$total=format_rupiah($row['total']);
$dibayar=format_rupiah($row['dibayar']);
$kembali=format_rupiah($row['kembali']);

$Data .= "Subtotal".$tab.$subtotal."\n";
$Data .= "PPN 10% ".$tab.$ppn."\n";
$Data .= "Total   ".$tab.$total."\n\n";
$Data .= "Dibayar ".$tab.$dibayar."\n";
$Data .= "Kembali ".$tab.$kembali."\n";
$Data .= "-------------------------------------------\n";
$Data .= "|  Terimakasih  | \n";
