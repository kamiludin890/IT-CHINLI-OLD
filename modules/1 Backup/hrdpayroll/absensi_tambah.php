...
$column_header='tanggal,nik,status_pegawai,nama,bagian';
...
if ($_POST['save_list']) {
	...
	$tahun_gaji=ambil_database(tahun_gaji,hrd_payroll_absensi,"id='$id'");
	$bulan_gaji=ambil_database(bulan_gaji,hrd_payroll_absensi,"id='$id'");
	$jumlah_hari=ambil_database(jumlah_hari,hrd_payroll_tunjangan,"bulan='".ambil_database(bulan_gaji,hrd_payroll_absensi,"id='$id'")."' AND tahun='".ambil_database(tahun_gaji,hrd_payroll_absensi,"id='$id'")."'");
	...
	$urut=0;for($i=0; $i < $nilai_column_id; ++$i){
		...
		$total_jam_kerja=ambil_database(total_jam_kerja,hrd_payroll_bagian,"bagian='$bagian'");
		$tambah_bpjs=ambil_database(tambah_bpjs,hrd_data_karyawan,"id='$id_pegawai'");
		$uang_profesional=ambil_database(uang_profesional,hrd_data_karyawan,"id='$id_pegawai'");


		$no_tgl=1;for($i_tgl=0; $i_tgl < $jumlah_hari; ++$i_tgl){
			$tgl_only=tampilan_tgl($no_tgl);
			$tanggal="$tahun_gaji-$bulan_gaji-$tgl_only";

			if (ambil_database(id_pegawai,hrd_payroll_absensi_items,"id_pegawai='$id_pegawai' AND tanggal='$tanggal'")=='') {
			mysql_query("INSERT INTO hrd_payroll_absensi_items SET
...
$result6=mysql_query("SELECT * FROM hrd_data_karyawan WHERE status_pegawai='Aktif' $if_pencarian");
$no=1;
