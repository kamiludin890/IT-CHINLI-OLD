...
function hapus($nama_database,$id,$nama_database_items){
	...
	if($nama_database=='hrd_payroll_absensi'){
		mysql_query("DELETE FROM hrd_payroll_absensi_items WHERE induk='$id'");
	}
...
//PENENTU UPDATE INSERT
					...
					if ($nama_database == 'hrd_payroll_tunjangan' OR $nama_database == 'hrd_payroll_bagian' OR $nama_database == 'hrd_payroll_absensi' OR $nama_database =='hrd_data_karyawan') {
...
//NOTICE INPUT END
	...
	if ($_POST['jenis'] == 'edit' AND $ijin_update == 'yes'){
			  $no=0;for($i=0; $i < $nilai_jumlah_pecahan; ++$i){
				...
				if ($nama_database=='hrd_payroll_tunjangan'){
						$bulan=ambil_database(bulan,$nama_database,"id='$id'");
						$tahun=ambil_database(tahun,$nama_database,"id='$id'");
						$gaji_pokok=ambil_database(gaji_pokok,$nama_database,"id='$id'");

						$jumlah_hari=jumlahhari($bulan,$tahun);
						$jumlah_sabtu=jumlah_sabtu_minggu("01-$bulan-$tahun","$jumlah_hari-$bulan-$tahun",6);
						$jumlah_minggu=jumlah_sabtu_minggu("01-$bulan-$tahun","$jumlah_hari-$bulan-$tahun",0);


						$bpjs=$gaji_pokok*1/100;
						$jamsostek=$gaji_pokok*2/100;
						$dana_pensiun=$gaji_pokok*1/100;

						mysql_query("UPDATE $nama_database SET jumlah_hari='$jumlah_hari',jumlah_sabtu='$jumlah_sabtu',jumlah_minggu='$jumlah_minggu',bpjs='$bpjs',jamsostek='$jamsostek',dana_pensiun='$dana_pensiun' WHERE id='$id'");
				}//aster Data Karyawan
	...
	if ($_POST['jenis'] == 'tambah' AND $ijin_tambah == 'yes'){
		...
		$bulan=ambil_database(bulan,$nama_database,"id='$ambil_id'");
		$tahun=ambil_database(tahun,$nama_database,"id='$ambil_id'");
		$gaji_pokok=ambil_database(gaji_pokok,$nama_database,"id='$ambil_id'");
		...
		mysql_query("UPDATE $nama_database SET jumlah_hari='$jumlah_hari',jumlah_sabtu='$jumlah_sabtu',jumlah_minggu='$jumlah_minggu',bpjs='$bpjs',jamsostek='$jamsostek',dana_pensiun='$dana_pensiun' WHERE id='$ambil_id'");
		}//DATA KARYAWAN



//OTOMATIS PINDAH LOKASI
if ($nama_database == 'hrd_data_karyawan') {
 ...
 //OTOMATIS PINDAH LOKASI SETELAH INSERT
...
echo "<table class='kolom_isi'>";
		...
		if ($pecah_column[$no] == 'tanggal' OR $pecah_column[$no] == 'tanggal_revisi'){$format_tgl="class='date' required";}else{$format_tgl="";}
...
//Status Nikah
elseif ($pecah_column[$no] == 'lanjut_kontrak' or $pecah_column[$no] == 'pemutusan_kontrak' or $pecah_column[$no] == 'id_card' or $pecah_column[$no] == 'status_vaksin2' or $pecah_column[$no] == 'status_vaksin' or $pecah_column[$no] == 'bersedia_keluar_kota' or $pecah_column[$no] == 'surat_pernyataan') {
	echo "<td>";
...
//BULAN PENGAJUAN
elseif ($pecah_column[$no] == 'bulan_gaji') {
	echo "<td>
	<select class='comboyuk' name='$pecah_column[$no]'>
	<option value='".$rows5[$pecah_column[$no]]."'>".$rows5[$pecah_column[$no]]."</option>";
	echo "<option value='01'>Januari</option>";
...
//Tahun Pengajuan
elseif ($pecah_column[$no] == 'tahun_gaji') {
	echo "
	<td><select class='comboyuk' name='$pecah_column[$no]'>";
	echo "<option value='".$rows5[$pecah_column[$no]]."'>".$rows5[$pecah_column[$no]]."</option>";
	$now=date('Y')+2;
...
//PAGING
				...
				//TAMPILAN RUPIAH
				elseif ($pecah_column_header[$no_items]=='uang_profesional' OR $pecah_column_header[$no_items]=='gaji_pokok' OR $pecah_column_header[$no_items]=='bonus_kehadiran' OR $pecah_column_header[$no_items]=='uang_makan' OR $pecah_column_header[$no_items]=='uang_transport' OR $pecah_column_header[$no_items]=='uang_shift' OR $pecah_column_header[$no_items]=='bpjs' OR $pecah_column_header[$no_items]=='jamsostek' OR $pecah_column_header[$no_items]=='dana_pensiun' OR $pecah_column_header[$no_items]=='thr') {
					$nama_gambar_tampilan=$rows1[$pecah_column_header[$no_items]];
					echo "<td style='background-color:$color;'>";
					echo rupiah($rows1[$pecah_column_header[$no_items]]);
					echo "</td>";
				}
        //TAMPILAN NORMAT TABEL
				...
				if ($nama_database == 'hrd_payroll_tunjangan' OR $nama_database == 'hrd_payroll_bagian' OR $nama_database == 'hrd_data_karyawan'){}else{
				...
				if ($rows1[validasi] == '') {
							if ($nama_database == 'hrd_data_karyawan'){}else{
