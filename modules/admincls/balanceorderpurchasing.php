<?php global $mod;
	$mod='admincls/balanceorderpurchasing';
...
function total_masuk_per_dokumen($id,$kodebarang){
...
// 	$total_masuk=$rows[masuk]+$total_masuk;}
 ...
 // $sql="SELECT SUM(masuk) AS tmasuk FROM inventory_distribusi_items WHERE induk='$id' AND kodebarang='$kodebarang' GROUP BY induk,kodebarang";
 // $result=mysql_query($sql);
 // $rows=mysql_fetch_array($result);
 // $total_masuk=$rows['tmasuk'];
...
$address='?mod=admincls/balanceorderpurchasing';
...
//PRINT EXCEL
echo "<a href='modules/admincls/cetak/print_excel_balance_order.php?pilihan_tahun=$pilihan_tahun&pilihan_bulan=$pilihan_bulan&pencarian=$pencarian&pilihan_pencarian=$pilihan_pencarian' target='_blank'><img src='modules/gambar/save_excel.png' width='25px'/></a></br>";
...
$result=mysql_query("SELECT * FROM admin_purchasing WHERE status='Proses' AND tanggal LIKE '$pilihan_tahun-$pilihan_bulan%' $if_pencarian ORDER BY tanggal");
$no=1;
while ($rows=mysql_fetch_array($result)){

$result1=mysql_query("SELECT * FROM admin_purchasing_items WHERE induk='$rows[id]'");
...
echo "<tr>";
		...
		$sql03="SELECT id_distribusi_masuk,qty FROM admin_purchasing_dokumen_masuk WHERE induk='$rows[id]' AND kodebarang='$rows1[kode_barang]'";
		...
		$nilai_id_distribusi=ambil_variabel_kutip_satu_where(admin_purchasing_dokumen_masuk,id_distribusi_masuk,"WHERE induk='$rows[id]'");
		$qty_nilai=ambil_variabel_satu_where(admin_purchasing_dokumen_masuk,qty,"WHERE id_distribusi_masuk IN ($nilai_id_distribusi) AND kodebarang='$rows1[kode_barang]' AND induk='$rows[id]'");
		$di_kurang=$rows1[qty_purchasing]-$qty_nilai;
		echo "<td>$qty_nilai</td>";//$qty_nilai

		if ($di_kurang=='0'){
			$color='#00FFFF';
			mysql_query("UPDATE sales_po SET status='Selesai' WHERE id='$rows[id]'"); 
		}elseif($di_kurang<0){$color='red';}else{$color='yellow';}
