...
//	elseif($_POST['mysubmit']=='edit'){echo usermenu('save,close');}
	...
	else{echo usermenu('mulai,delete,filter');}
	}
...
function editform($id,$btn){ extract($GLOBALS); 
	...
	echo "<script type='text/javascript'>window.location.href='?menu=home&mod=pos/kasir_items&res=h'</script>";
	}
 	}	


function mulai(){ extract($GLOBALS);
$query="INSERT INTO pos_transaksi SET 
	...
	$id=mysql_insert_id();
	$_SESSION['idinduk']=$id; 
	echo "<script type='text/javascript'>window.location.href='?menu=home&mod=pos/kasir_items'</script>";
