...
function rupiah_pembulatan_round($nilai){
$angka=$nilai;//round($nilai)
$hasil_rupiah = "Rp " . number_format($angka,2,',','.');
return $hasil_rupiah;}
...
function hapus($nama_database,$id,$nama_database_items){
	...
	if ($nama_database=='akuntansi_posting_master') {
		$string_delete_jurnal="DELETE FROM akuntansi_jurnal WHERE induk_master='$id'";
...
//PENENTU UPDATE INSERT
					...
					if ($nama_database == 'akuntansi_persamaan') {
						$ijin_update='yes'; $ijin_tambah='yes';
					}//Sales BUkan Penentu Update

					if ($nama_database == 'akuntansi_posting_master') {
						$ref=ambil_database(ref,akuntansi_posting_master,"id='$_POST[ref]'");
						if ($ref) {
	...
	if ($_POST['jenis'] == 'edit' AND $ijin_update == 'yes'){
			  $no=0;for($i=0; $i < $nilai_jumlah_pecahan; ++$i){
				...
				if ($nama_database=='akuntansi_posting_master'){
					...
					mysql_query("UPDATE akuntansi_posting_master SET validasi='',status='' WHERE id='$id'");
				}//INVOICE
...
//status
elseif ($pecah_column[$no] == 'status' AND $nama_database=='akuntansi_persamaan'){
echo "<td>
<select class='comboyuk' name='$pecah_column[$no]' style='width:100%;'>
<option value='".$rows5[$pecah_column[$no]]."'>".$rows5[$pecah_column[$no]]."</option>";
...
elseif ($pecah_column[$no] == 'persamaan') {
	$sql113="SELECT * FROM akuntansi_persamaan";
	$result113=mysql_query($sql113);
	echo "<td>
	<select class='comboyuk' name='$pecah_column[$no]' style='width:450px;'>
	<option value='".$rows5[$pecah_column[$no]]."'>".ambil_database(kelompok,akuntansi_persamaan,"id='".$rows5[$pecah_column[$no]]."'")." | ".ambil_database(keterangan,akuntansi_persamaan,"id='".$rows5[$pecah_column[$no]]."'")."</option>";
		while ($rows113=mysql_fetch_array($result113)) {
...
elseif ($pecah_column[$no] == 'debit' OR $pecah_column[$no] == 'kredit' OR $pecah_column[$no] == 'kredit_kedua') {
	$sql113="SELECT * FROM akuntansi_akun ORDER BY id";
	$result113=mysql_query($sql113);
	echo "<td>
	<select class='comboyuk' name='$pecah_column[$no]' style='width:450px;'>
	<option value='".$rows5[$pecah_column[$no]]."'>".$rows5[$pecah_column[$no]]."</option>";
		while ($rows113=mysql_fetch_array($result113)) {
	echo "<option value='$rows113[nomor]'>$rows113[kelompok] | $rows113[nomor] | $rows113[nama]</option>";}
	echo "
	</select>
	</td>";
}
...
if ($total_tampil_tambah=='1') {
  echo "<a href='$address&id=".base64_encrypt($rows1[id],"XblImpl1!A@")."&opsi=".base64_encrypt("tambah","XblImpl1!A@")."&halaman=$nomor_halaman&pilihan_tahun=$pilihan_tahun&pilihan_bulan=$pilihan_bulan&pilihan_pencarian=$pilihan_pencarian&pencarian=$pencarian'><img src='modules/gambar/tambah.png' width='25px'/></a></br>";
			...
			$no++;}
					...
					if ($nama_database=='akuntansi_persamaan' OR $nama_database=='akuntansi_posting_master') {
						echo "<th colspan='3' style=''><strong>".ambil_database($bahasa,master_bahasa,"kode='Opsi'")."</strong></th>";
					}
				...
				if ($address=='?mod=financecl/packinglistsheetyardmeter') {$jenis_format_sheet="AND jenis_satuan='SHEETYARDMETER'";}else{$jenis_format_sheet='';}
				if ($address=='?mod=financecl/packinglistpairs') {$jenis_format_pairs="AND jenis_satuan='PAIRS'";}else{$jenis_format_pairs='';}
				if ($address=='?mod=financecl/packinglistpcs') {$jenis_format_pcs="AND jenis_satuan='PCS'";}else{$jenis_format_pcs='';}
				if ($_SESSION['format_pt']!='' AND $address=='?mod=financecl/packinglistpcs' OR $_SESSION['format_pt']!='' AND $address=='?mod=financecl/packinglistpairs'){$tampil_format_pt="AND perusahaan='$_SESSION[format_pt]'";}else{$tampil_format_pt='';}
...
$result = mysql_query("SELECT	* FROM $nama_database WHERE tanggal LIKE '$pilihan_tahun-$pilihan_bulan%' $jenis_format_pcs $jenis_format_sheet $jenis_format_pairs $if_pencarian $tampil_format_pt");
$total = mysql_num_rows($result);
$pages = ceil($total/$halaman);
$query = mysql_query("SELECT	* FROM $nama_database WHERE tanggal LIKE '$pilihan_tahun-$pilihan_bulan%' $jenis_format_pcs $jenis_format_sheet $jenis_format_pairs $if_pencarian $tampil_format_pt ORDER BY tanggal DESC LIMIT $mulai, $halaman")or die(mysql_error);
$no =$mulai+1;
//PAGING
				...
				elseif ($pecah_column_header[$no_items]=='persamaan') {
					echo "<td style='background-color:$color;'>".ambil_database(kelompok,akuntansi_persamaan,"id='".$rows1[$pecah_column_header[$no_items]]."'")." | ".ambil_database(keterangan,akuntansi_persamaan,"id='".$rows1[$pecah_column_header[$no_items]]."'")."</td>";
				}
...
//				if ($rows1[validasi] == 'NOT SHOW') {
						if ($nama_database == 'akuntansi_persamaan' OR $nama_database == 'akuntansi_posting_master' AND ambil_database(status,$nama_database,"id='$rows1[id]'")!='Selesai'){
...
//			 }else{}//echo "<td style='width:10 background-color:$color;'></td>";
				...
				if ($rows1[validasi] == 'NOT SHOW' OR $nama_database=='akuntansi_persamaan' OR $nama_database == 'akuntansi_posting_master' AND ambil_database(status,$nama_database,"id='$rows1[id]'")!='Selesai') {
							if ($nama_database == 'akuntansi_persamaan' OR $nama_database == 'akuntansi_posting_master'){
				...
				if($rows1[validasi] == Valid AND $validasi == ya AND $nama_database == akuntansi_posting_master){
				echo "<td style='text-align: center; background-color: white; border-top-right-radius: 10px; border-bottom-right-radius: 10px;'><center>";
