...
function hapus($nama_database,$id,$nama_database_items,$address){
	...
	if ($nama_database=='akuntansiv2_posting_master') {
		$string_delete_jurnal="DELETE FROM akuntansiv2_jurnal WHERE induk_master='$id'";
...
//PENENTU UPDATE INSERT
					...
					if ($nama_database == 'akuntansiv2_persamaan' OR $nama_database == 'akuntansiv2_akun') {
						$ijin_update='yes'; $ijin_tambah='yes';
					}//Sales BUkan Penentu Update

					if ($nama_database == 'akuntansiv2_posting_master') {
						$ref=ambil_database(ref,akuntansiv2_posting_master,"id='$_POST[ref]'");
						if ($ref) {
	...
	if ($_POST['jenis'] == 'edit' AND $ijin_update == 'yes'){
			  $no=0;for($i=0; $i < $nilai_jumlah_pecahan; ++$i){
				...
				if ($nama_database=='akuntansiv2_posting_master'){
					...
					//UPDATE INTO
					mysql_query("UPDATE akuntansiv2_posting SET
						induk='$id',
						kode='$kode',
						tanggal='$tanggal',
					...
					$no_invoice=ambil_database(no_invoice,$nama_database,"id='$id'");
					mysql_query("UPDATE akuntansiv2_posting_master SET validasi='',status='' WHERE id='$id'");
				}//INVOICE
...
//status
elseif ($pecah_column[$no] == 'status' AND $nama_database=='akuntansiv2_persamaan'){
echo "<td>
<select class='comboyuk' name='$pecah_column[$no]' style='width:100%;'>
<option value='".$rows5[$pecah_column[$no]]."'>".$rows5[$pecah_column[$no]]."</option>";
...
elseif ($pecah_column[$no] == 'persamaan') {
	$sql113="SELECT * FROM akuntansiv2_persamaan";
	$result113=mysql_query($sql113);
	echo "<td>
	<select class='comboyuk' name='$pecah_column[$no]' style='width:450px;'>
	<option value='".$rows5[$pecah_column[$no]]."'>".ambil_database(kelompok,akuntansiv2_persamaan,"id='".$rows5[$pecah_column[$no]]."'")." | ".ambil_database(keterangan,akuntansiv2_persamaan,"id='".$rows5[$pecah_column[$no]]."'")."</option>";
		while ($rows113=mysql_fetch_array($result113)) {
...
elseif ($pecah_column[$no] == 'debit' OR $pecah_column[$no] == 'kredit' OR $pecah_column[$no] == 'kredit_kedua') {
	$sql113="SELECT * FROM akuntansiv2_akun WHERE master NOT LIKE 'ya' ORDER BY id";
	$result113=mysql_query($sql113);
	echo "<td>
	<select class='comboyuk' name='$pecah_column[$no]' style='width:450px;'>
	<option value='".$rows5[$pecah_column[$no]]."'>".ambil_database(kelompok,akuntansiv2_akun,"nomor='".$rows5[$pecah_column[$no]]."'")." | ".ambil_database(nomor,akuntansiv2_akun,"nomor='".$rows5[$pecah_column[$no]]."'")." | ".ambil_database(nama,akuntansiv2_akun,"nomor='".$rows5[$pecah_column[$no]]."'")."</option>";
		while ($rows113=mysql_fetch_array($result113)) {
...
//Alamat
elseif ($pecah_column[$no] == 'master' AND $nama_database=='akuntansiv2_akun') {
	echo "<td>";
...
$no=0;
$query_duplikat=mysql_query("SELECT nomor, COUNT(*) duplikat FROM akuntansiv2_akun GROUP BY nomor HAVING COUNT(duplikat) > 1");
	...
	if ($result_duplikat[nomor]) {
		echo "<h4 style='background-color:yellow; width:200px;'>Terjadi Duplikat Data di Nomor $result_duplikat[nomor]</h4>";
			...
			$no=0;for($i=0; $i < $nilai_jumlah_pecahan_header; ++$i){
					...
					//NOMOR jadi AKUN
					if ($pecah_column_header[$no]=='nomor' AND $nama_database=='akuntansiv2_akun') {
			...
			$no++;}
					...
					if ($nama_database=='akuntansiv2_persamaan' OR $nama_database=='akuntansiv2_posting_master' OR $nama_database=='akuntansiv2_akun'OR $nama_database=='deliverycl_invoice' OR $nama_database=='akuntansiv2_jurnal') {
				...
				if ($address=='?menu=home&mod=akuntansiv2/akun') {$order_by="nomor,id";}else{$order_by="tanggal,id";}
...
$query = mysql_query("SELECT	* FROM $nama_database WHERE tanggal LIKE '$pilihan_tahun-$pilihan_bulan%' $if_pencarian ORDER BY $order_by ASC LIMIT $mulai, $halaman")or die(mysql_error);
$no =$mulai+1;
//PAGING
				...
				elseif ($pecah_column_header[$no_items]=='persamaan') {
					echo "<td style='background-color:$color;'>".ambil_database(kelompok,akuntansiv2_persamaan,"id='".$rows1[$pecah_column_header[$no_items]]."'")." | ".ambil_database(keterangan,akuntansiv2_persamaan,"id='".$rows1[$pecah_column_header[$no_items]]."'")."</td>";
				}
...
//				if ($rows1[validasi] == 'NOT SHOW') {
						if ($nama_database == 'akuntansiv2_persamaan' OR $nama_database == 'akuntansiv2_akun' OR $nama_database == 'akuntansiv2_posting_master' AND ambil_database(status,$nama_database,"id='$rows1[id]'")!='Selesai'){
...
//			 }else{}//echo "<td style='width:10 background-color:$color;'></td>";
				...
				if ($nama_database == 'akuntansiv2_posting_master' OR $nama_database == 'deliverycl_invoice'){
				...
				if ($rows1[validasi] == 'NOT SHOW' OR $nama_database=='akuntansiv2_persamaan' OR $nama_database == 'akuntansiv2_posting_master'  OR $nama_database == 'akuntansiv2_jurnal' AND ambil_database(status,$nama_database,"id='$rows1[id]'")!='Selesai' OR $nama_database == 'akuntansiv2_akun') {
							if ($nama_database == 'akuntansiv2_persamaan' OR $nama_database == 'akuntansiv2_posting_master' OR $nama_database == 'akuntansiv2_akun' OR $nama_database == 'akuntansiv2_jurnal'){
