...
//
// 	echo "
// 	<form action='?menu=home&mod=akuntansiv2/bukubesar' method='POST'>
// 	</br>
// 	Bulan
// 	<select name='bulan'>
...
//
// echo "</br>
// <form method ='post' action='?menu=home&mod=akuntansiv2/bukubesar'>
...
if ($list_buku_besar_dokumen==list_buku_besar_dokumen) {
  ...
  $rows4=mysql_fetch_array($result4);
	echo "
			...
			echo "<form method ='post' action='?menu=home&mod=akuntansiv2/bukubesar'>
			<table>
...
$query28="SELECT SUM(bayar) FROM akuntansiv2_posting WHERE kontak='$rows1[kontak]' AND tanggal LIKE '%$rows22[tanggal_pencarian_buku_besar]%' AND jenis_doc IN ('BC 2.7 MASUK','BC 4.0 LOKAL','BC 2.3 IMPORT','BC 2.6.2 HASIL SUBKON')";
$result28=mysql_query($query28);
$row28=mysql_fetch_array($result28);
$total_kredit_posting=$row28['SUM(bayar)'];

$query29="SELECT SUM(bayar) FROM akuntansiv2_posting WHERE kontak='$rows1[kontak]' AND tanggal LIKE '%$rows22[tanggal_pencarian_buku_besar]%' AND jenis_doc IN ('BC 3.0 PEB','BC 2.5 PIB','BC 2.6.1 SUBKON','BC 2.7 KELUAR','BC 4.1 PENJUALAN')";
...
echo "
		...
		$sql54="SELECT * FROM akuntansiv2_akun WHERE nomor='$_POST[akun]'";
		$result54= mysql_query($sql54);
		$rows54=mysql_fetch_array($result54);
		$nomorakun=$rows54['nomor'];
		$namaakun=$rows54['nama'];

		$sql53="SELECT * FROM akuntansiv2_akun ORDER BY nomor";
		$result53= mysql_query($sql53);
		echo "
	...
	if (isset($_POST['akun']) ){
	$akun=$_POST['akun'];
	$bb = getrow("nomor,nama","akuntansiv2_akun","where nomor=$akun");
...
//Pilihan Akun Awal"
 	//$result1=mysql_query("SELECT nomor, nama FROM akuntansiv2_akun ");
...
if ($akun=='01') {
	...
	$query23="SELECT SUM(debit) FROM akuntansiv2_jurnal WHERE $akun_all AND tanggal BETWEEN '$rows23[saldo_awal_buku_besar1]' AND '$rows23[saldo_awal_buku_besar2]'";
	$result23=mysql_query($query23);
	$row23=mysql_fetch_array($result23);

	$query24="SELECT SUM(kredit) FROM akuntansiv2_jurnal WHERE $akun_all AND tanggal BETWEEN '$rows23[saldo_awal_buku_besar1]' AND '$rows23[saldo_awal_buku_besar2]'";
	...
	$row31=mysql_fetch_array($result31);
			...
			</br><li><a href='modules/akuntansiv2/bukubesar_print.php?akun=$akun&username=admin' target='_blank'>Ekspor To Excel</a></li>
...
$url = "http://localhost/main.php?email=$email_address&event_id=$event_id";
			...
			<td>$data[ref]</td>
			<td>$data[nomor] - ".ambil_database(nama,akuntansiv2_akun,"nomor='$data[nomor]'")."</td>
