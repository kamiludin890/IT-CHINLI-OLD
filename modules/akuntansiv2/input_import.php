...
if ($jenis_import=='SALES') {
				...
				for ($i=2; $i<=$jumlah_baris; $i++){
					...
					$kode_voucher=$data->val($i, 8, 0);

					$keterangan_debit=ambil_database(nama,akuntansiv2_akun,"nomor='$debit'");
					$pembeda_neraca_debit=ambil_database(pembeda_neraca,akuntansiv2_akun,"nomor='$debit'");
					$pembeda_laba_rugi_debit=ambil_database(pembeda_laba_rugi,akuntansiv2_akun,"nomor='$debit'");

					$keterangan_kredit=ambil_database(nama,akuntansiv2_akun,"nomor='$kredit'");
					$pembeda_neraca_kredit=ambil_database(pembeda_neraca,akuntansiv2_akun,"nomor='$kredit'");
					$pembeda_laba_rugi_kredit=ambil_database(pembeda_laba_rugi,akuntansiv2_akun,"nomor='$kredit'");

					$nama_barang=$data->val($i, 9, 0);
					$perusahaan=$data->val($i, 10, 0);
	...
	//RECEIVE SALES
	if ($jenis_import=='RECEIVE SALES' OR $jenis_import=='DISCOUNT SALES') {
					...
					for ($i=2; $i<=$jumlah_baris; $i++){
						...
						$faktur=$data->val($i, 3, 0);
						$tanggal_faktur=$data->val($i, 4, 0);
						$debit=$data->val($i, 5, 0);
						$kredit=$data->val($i, 6, 0);
						$nilai=$data->val($i, 7, 0);
						$receive_date=$data->val($i, 8, 0);
						$kode_voucher=$data->val($i, 9, 0);
						...
						$nama_barang=$data->val($i, 10, 0);
						$perusahaan=$data->val($i, 11, 0);

						mysql_query("INSERT INTO akuntansiv2_jurnal SET
							induk_invoice='$invoice',
							ref='$faktur',
							tanggal='$tanggal_faktur',
							tanggal_diterima='$receive_date',
							tanggal_input='$tanggal_hari_ini',
							nomor='$debit',
							nama='DEBIT',
							keterangan='$keterangan_debit',
