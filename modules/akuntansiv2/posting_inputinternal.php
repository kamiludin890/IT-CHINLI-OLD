...
function tamount_bayar($nomor_aju,$jenis_doc){
	$result5=mysql_query("SELECT bayar FROM akuntansiv2_posting WHERE nomor_aju='$nomor_aju' AND jenis_doc='$jenis_doc' AND pencatatan NOT LIKE 'ya'");
...
function tamount_ppn($nomor_aju,$jenis_doc){
	$result5=mysql_query("SELECT ppn FROM akuntansiv2_posting WHERE nomor_aju='$nomor_aju' AND jenis_doc='$jenis_doc'");
	while ($rows5=mysql_fetch_array($result5)) {
...
function gdbb_produksi($id,$kegiatan){
	$result4=mysql_query("SELECT * FROM inventory_distribusi_items WHERE induk='$id'");
	while ($rows4=mysql_fetch_array($result4)) {
			$hasil_bagi=ambil_database(nilai,inventory_distribusi_items,"id='$rows4[id_scanner]'")/ambil_database(masuk,inventory_distribusi_items,"id='$rows4[id_scanner]'");
			$hasil_kali=$hasil_bagi*$rows4[$kegiatan];
			$nilai=$hasil_kali+$nilai;
	}
return $nilai;}


function produksi_gdbj($id){
	$result4=mysql_query("SELECT * FROM inventory_distribusi_items WHERE induk='$id'");
	while ($rows4=mysql_fetch_array($result4)) {
		$id_laminating=ambil_database(id_laminating,planningcls_spkcuttingdies,"no_spk_cutting='$rows4[nomor_spk]'");
		$nomor_spk_laminating=ambil_database(no_spk,planningcls_spklaminating,"id='".ambil_database(induk,planningcls_spklaminating_items,"id='$id_laminating'")."'");
		$result5=mysql_query("SELECT * FROM inventory_distribusi_items WHERE nomor_spk IN ('$rows4[nomor_spk]','$nomor_spk_laminating') AND kode NOT REGEXP '-IN-WL|-OUT-WL|-IN-WC|-OUT-WC'");
		while ($rows5=mysql_fetch_array($result5)) {
				$hasil_bagi=ambil_database(nilai,inventory_distribusi_items,"id='$rows5[id_scanner]'")/ambil_database(masuk,inventory_distribusi_items,"id='$rows5[id_scanner]'");
				$hasil_kali=$hasil_bagi*$rows5[keluar];
				$nilai=$hasil_kali+$nilai;
		}
	}
return $nilai;}
...
echo "<html>
<meta charset='UTF-8'>
<head><title>Dokumen Internal</title></head>
<body>";
//END TITLE

$column_header='kode,jenis_doc,bukti,tanggal_bukti,kegiatan,kontak,keterangan';
...
$tanggal=ambil_database(tanggal,akuntansiv2_posting_master,"id='$id'");
$kode=ambil_database(kode,akuntansiv2_posting_master,"id='$id'");
$keterangan=ambil_database(keterangan,akuntansiv2_posting_master,"id='$id'");
$id_persamaan=ambil_database(persamaan,akuntansiv2_posting_master,"id='$id'");
$persamaan=ambil_database(keterangan,akuntansiv2_persamaan,"id='".ambil_database(persamaan,akuntansiv2_posting_master,"id='$id'")."'");
$debit=ambil_database(debit,akuntansiv2_persamaan,"id='".ambil_database(persamaan,akuntansiv2_posting_master,"id='$id'")."'");
$kredit=ambil_database(kredit,akuntansiv2_persamaan,"id='".ambil_database(persamaan,akuntansiv2_posting_master,"id='$id'")."'");
$kredit_kedua=ambil_database(kredit_kedua,akuntansiv2_persamaan,"id='".ambil_database(persamaan,akuntansiv2_posting_master,"id='$id'")."'");
$pembuat=ambil_database(pembuat,akuntansiv2_posting_master,"id='$id'");
$keterangan_debit=ambil_database(nama,akuntansiv2_akun,"nomor='$debit'");
$keterangan_kredit=ambil_database(nama,akuntansiv2_akun,"nomor='$kredit'");
$keterangan_kredit_kedua=ambil_database(nama,akuntansiv2_akun,"nomor='$kredit_kedua'");
...
$jenis_kas=ambil_database(jenis_kas,akuntansiv2_posting_master,"id='$id'");
$tanggal_input=ambil_database(tgl_dibuat,akuntansiv2_posting_master,"id='$id'");
...
$pecah_id_harga=pecah(str_replace('@',',',$jumlah_column_id[$urut]));
//ISIAN KOLOM DARI ISIAN
$id_terpilih=$pecah_id_harga[0];
...
$nomor_aju=ambil_database(nomor_aju,inventory_distribusi,"id='$id_terpilih'");
	...
	$nilai_bayar=$pecah_id_harga[1];
	$nilai_sudah_dibayarkan=tamount_bayar($nomor_aju,$jenis_doc);
	$nominal_debit=$nilai_bayar;//ambil_database(harga_penyerahan,inventory_distribusi,"id='$id_terpilih'")-$nilai_sudah_dibayarkan;
	$nominal_kredit=$nilai_bayar;//ambil_database(harga_penyerahan,inventory_distribusi,"id='$id_terpilih'")-$nilai_sudah_dibayarkan;
...
//INSERT INTO
mysql_query("INSERT INTO akuntansiv2_posting SET
	induk='$id',
	kode='$kode',
	tanggal='$tanggal',
...
echo "<table class='tabel_utama' style='width:auto;'>";
	...
	//ISI TABEL
		...
		$mulai = ($page>1) ? ($page * $halaman) - $halaman : 0;//('%-IN-WL','%-OUT-WL','%-IN-WC','%-OUT-WC')
		$result = mysql_query("SELECT * FROM inventory_distribusi WHERE tanggal LIKE '$pilihan_tahun-$pilihan_bulan%' AND jenis_doc	NOT LIKE 'BC %' AND kode NOT REGEXP '-IN-WL|-OUT-WL|-IN-WC|-OUT-WC' $if_pencarian");
		$total = mysql_num_rows($result);
		$pages = ceil($total/$halaman);
		$query = mysql_query("SELECT * FROM inventory_distribusi WHERE tanggal LIKE '$pilihan_tahun-$pilihan_bulan%' AND jenis_doc	NOT LIKE 'BC %' AND kode NOT REGEXP '-IN-WL|-OUT-WL|-IN-WC|-OUT-WC' $if_pencarian ORDER BY id ASC LIMIT $mulai, $halaman")or die(mysql_error);
		$urut=$mulai+1;
		//PAGING
		...
		while ($rows=mysql_fetch_array($query)) {
			...
			echo "<td>$urut</td>";
				...
				echo "<td style='text-align:right; height:25px;'>";
						//GDBB TO LAMINATING AND GDBB TO CUTTING
						if ($rows['jenis_doc']=='GDBB TO LAMINATING' OR $rows['jenis_doc']=='GDBB TO CUTTING') {
							echo rupiah(gdbb_produksi($rows[id],$rows[kegiatan]));
							$nilai_bayar=gdbb_produksi($rows[id],$rows[kegiatan]);
						}
						//GDBB TO LAMINATING AND GDBB TO CUTTING END


						//CUTTING TO GDBJ
						if ($rows['jenis_doc']=='CUTTING TO GDBJ') {
							echo rupiah(produksi_gdbj($rows[id]));
							$nilai_bayar=produksi_gdbj($rows[id]);
						}
						//CUTTING TO GDBJ END
				echo "</td>";

				echo "<td><input type='checkbox' name='id_terpilih[]' value='$rows[id]@$nilai_bayar'></td>";
			echo "</tr>";
		$urut++;}
	//ISI TABEL END
