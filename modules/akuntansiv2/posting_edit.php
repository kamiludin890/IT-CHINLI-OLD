...
function tamount_bayar($nomor_aju,$jenis_doc,$id_posting){
	$result5=mysql_query("SELECT bayar FROM akuntansiv2_posting WHERE nomor_aju='$nomor_aju' AND jenis_doc='$jenis_doc' AND id NOT LIKE '$id_posting'");
...
//}else {
	...
	mysql_query("UPDATE akuntansiv2_posting SET bayar='$bayar[$no]',nilai_ppn='$nilai_ppn[$no]',jumlah_ppn='$hasil_bayar',catatan='$catatan[$no]',ppn='$total_ppn',nominal_debit='$hasil_bayar',nominal_kredit='$hasil_bayar',pencatatan='$pencatatan[$no]' WHERE id='$id_item[$no]'");
//}
...
if ($id_delete) {
	$no=0;for($i=0; $i < count($_POST['id_delete']); ++$i){
		if ($id_delete[$no]){
			mysql_query("DELETE FROM akuntansiv2_posting WHERE id='$id_delete[$no]'");
		}
	$no++;}
}//DELETE END
...
$result=mysql_query("SELECT * FROM akuntansiv2_posting WHERE induk='$id'");
$urut=1;
while ($rows=mysql_fetch_array($result)) {
