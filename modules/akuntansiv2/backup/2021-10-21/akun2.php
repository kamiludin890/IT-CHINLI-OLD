<?php global  $title, $mod, $tbl, $fld, $akses ;
	$mod='akuntansiv2/akun';
	$tbl='akuntansiv2_akun';
	...
	function editmenu(){extract($GLOBALS);
		...
		function home(){extract($GLOBALS);
			...
			if ($idhal=='') {
				...
				echo "
				<form action='?menu=home&mod=akuntansiv2/akun' method='POST'>
									...
									<table align='center' width='100%' border='2' cellpadding='3' cellspacing='1' bgcolor='#CCCCCC'>
										<tr>
											...
											<td style='padding: 5px; color:black; background-color: #E0FFFF;' align='center' width='40px' bgcolor='#FFFFFF'><strong></strong></td>
												...
												$sql1="SELECT * FROM akuntansiv2_akun WHERE tampil='' order by nomor";
			...
			if ($idhal>0) {
				...
				$sql2="SELECT * FROM akuntansiv2_akun WHERE id='$idhal' order by nama";
									...
									<table align='center' width='100%' border='2' cellpadding='3' cellspacing='1' bgcolor='#CCCCCC'>
										<tr>
											...
											<td style='padding: 5px; color:black; background-color: #E0FFFF;' align='center' width='40px' bgcolor='#FFFFFF'><strong></strong></td>
												...
												$sql3="SELECT * FROM akuntansiv2_akun WHERE tampil='yes' AND nomor LIKE '%$rows2[nomor].%' ORDER BY nomor";
		...
		if ($idhal==230) {
			echo "
			<form method ='post' action='?menu=home&mod=akuntansiv2/akun'>
			  <table>
				<tr>
				<td>Kelompok</td>
				<td>:</td>
			...
			$upload = "INSERT IGNORE INTO akuntansiv2_akun (kelompok,nomor,nama,induk,kolom,pembeda_laba_rugi) VALUES('$kelompok','$nomor','$nama','$nomor','$kolom','$pembeda_laba_rugi')";
		...
		if ($idhal==220) {
			...
			$upload = "DELETE FROM akuntansiv2_akun WHERE id='$iddeletehome'";
			$hasil = mysql_query($upload);
			echo "
		...
		if ($idhal==210) {
			$iddeletehome=$_POST['iddeletehome'];
			$sql4="SELECT * FROM akuntansiv2_akun WHERE id='$iddeletehome'";
			...
			$kolom=$_POST['kolom'];
			$pembeda_laba_rugi=$_POST['pembeda_laba_rugi'];
			$upload = "UPDATE akuntansiv2_akun SET pembeda_laba_rugi='$pembeda_laba_rugi',kolom='$kolom',kelompok='$kelompok',nomor='$nomor',nama='$nama' WHERE id='$iddeletehome' ";
			$hasil = mysql_query($upload);
			echo "
		...
		if ($idhal==130) {
			...
			$iddaftarproduk=$_POST['iddaftarproduk'];
			$upload = "INSERT IGNORE INTO akuntansiv2_akun (kolom,pembeda_laba_rugi,kelompok,nomor,nama,tampil,induk) VALUES('$kolom','$pembeda_laba_rugi','$kelompok','$nomor','$nama','$tampil','$nomorinduk')";
			$hasil = mysql_query($upload);
			echo "
				<h2>Data Telah Disimpan</h2>
			  <form action='?menu=home&mod=akuntansiv2/akun' method='POST'>
		...
		if ($idhal==110) {
			...
			$iddaftarproduk=$_POST['iddaftarproduk'];
			$upload = "DELETE FROM akuntansiv2_akun WHERE id='$iddeletedaftar'";
			$hasil = mysql_query($upload);
			echo "
				<h2>Data Telah Dihapus</h2>
			  <form action='?menu=home&mod=akuntansiv2/akun' method='POST'>
		...
		if ($idhal==120) {
			...
			$kolom=$_POST['kolom'];
			$pembeda_laba_rugi=$_POST['pembeda_laba_rugi'];
			$upload = "UPDATE akuntansiv2_akun SET kolom='$kolom',pembeda_laba_rugi='$pembeda_laba_rugi',kelompok='$kelompok',nomor='$nomor',nama='$nama' WHERE id='$iddeletehome' ";
			$hasil = mysql_query($upload);
			echo "
				<h2>Data Telah di Update</h2>
			  <form action='?menu=home&mod=akuntansiv2/akun' method='POST'>
...
function doimport(){ extract($GLOBALS);
	...
	$query = "INSERT INTO akuntansiv2_akun VALUES ('$id','$kelompok','$nomor','$nama')";
