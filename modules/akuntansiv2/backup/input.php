...
if (ambil_database(induk_invoice,akuntansiv2_jurnal,"induk_invoice='$id_dipilih1'")=='' AND $debit1!='' AND $kredit1!='') {
		//DEBIT
		$keterangan_debit=ambil_database(nama,akuntansiv2_akun,"nomor='$debit1'");
		$pembeda_saldo_awal_debit=ambil_database(pembeda_neraca,akuntansiv2_akun,"nomor='$debit1'");
		$pembeda_laba_rugi_debit=ambil_database(pembeda_laba_rugi,akuntansiv2_akun,"nomor='$debit1'");
		...
		//KREDIT
		$keterangan_kredit=ambil_database(nama,akuntansiv2_akun,"nomor='$kredit1'");
		$pembeda_saldo_awal_kredit=ambil_database(pembeda_neraca,akuntansiv2_akun,"nomor='$kredit1'");
		$pembeda_laba_rugi_kredit=ambil_database(pembeda_laba_rugi,akuntansiv2_akun,"nomor='$kredit1'");
				mysql_query("INSERT INTO akuntansiv2_jurnal SET
					ref='$kode_voucher1',
					induk_invoice='$id_dipilih1',
					tanggal='$tanggal1',
...
}elseif(ambil_database(induk_invoice,akuntansiv2_jurnal,"induk_invoice='$id_dipilih1'")!='' AND $debit1!='' AND $kredit1!=''){
	...
	$pembeda_saldo_awal_debit=ambil_database(pembeda_neraca,akuntansiv2_akun,"nomor='$debit1'");
	$pembeda_laba_rugi_debit=ambil_database(pembeda_laba_rugi,akuntansiv2_akun,"nomor='$debit1'");
			mysql_query("UPDATE akuntansiv2_jurnal SET
				ref='$kode_voucher1',
				induk_invoice='$id_dipilih1',
				tanggal='$tanggal1',
				tanggal_input='$tanggal_input',
	...
	while ($rows1=mysql_fetch_array($query)){
	  ...
	  <select class='comboyuk' style='width:150px;' name='debit[]'>
		<option value='".ambil_database(nomor,akuntansiv2_jurnal,"induk_invoice='$rows1[id]' AND kredit=''")."'>".ambil_database(nomor,akuntansiv2_jurnal,"induk_invoice='$rows1[id]' AND kredit=''")." | ".ambil_database(keterangan,akuntansiv2_jurnal,"induk_invoice='$rows1[id]' AND kredit=''")."</option>
	  <option value=''></option>";
	  ...
	  echo "
		...
		<option value='".ambil_database(nomor,akuntansiv2_jurnal,"induk_invoice='$rows1[id]' AND debit=''")."'>".ambil_database(nomor,akuntansiv2_jurnal,"induk_invoice='$rows1[id]' AND debit=''")." | ".ambil_database(keterangan,akuntansiv2_jurnal,"induk_invoice='$rows1[id]' AND debit=''")."</option>
		<option value=''></option>";
			while ($rows113=mysql_fetch_array($result113)) {
		echo "<option value='$rows113[nomor]'>$rows113[nomor] | $rows113[nama]</option>";}
		echo "
		...
		echo "<td><input type='text' style='width:75px;' class='date' name='tanggal[]' value='".ambil_database(tanggal,akuntansiv2_jurnal,"induk_invoice='$rows1[id]'")."'></td>";

		echo "<td><input type='text' style='width:75px;' name='kode_voucher[]' value='".ambil_database(ref,akuntansiv2_jurnal,"induk_invoice='$rows1[id]'")."'></td>";
...
//START UTAMA
	...
	echo "<table>";
		echo "<tr>";
		echo "<td><form method ='POST' action='modules/akuntansiv2/cetak/print_excel_efaktur.php' target='_blank'>";
