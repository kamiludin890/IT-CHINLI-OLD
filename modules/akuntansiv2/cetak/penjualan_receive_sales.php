...
//PECAH
$column_items='No,Invoice,Faktur,Faktur Date,Debit,Kredit,Amount,Receive Date,Kode Voucher,Nama Barang,Nama Perusahaan';
...
while ($rows1=mysql_fetch_array($query)){
	...
	$objPHPExcel->getActiveSheet()->setCellValue("G$no", "$rows1[amount_rp]");
	$objPHPExcel->getActiveSheet()->setCellValue("H$no", "YYYY-MM-DD");
	$objPHPExcel->getActiveSheet()->setCellValue("J$no", "$rows1[item_name]");
	$objPHPExcel->getActiveSheet()->setCellValue("K$no", "$rows1[perusahaan]");
...
header("Content-Disposition: attachment;filename=Receive Sales $periode1 s/d $periode2.xls");
