...
function buku_besar(){ extract($GLOBALS);
	...
	if (isset($_POST['akun']) ){
		...
		$rows1=mysql_fetch_array($result1);
			...
			echo "<form action='?menu=home&mod=akuntansiv2/bukubesar' method='POST'>
						<label>Tanggal</label>
	...
	$datasec=$_POST['test'];
	if (isset($datasec)) { $data=implode(",",$datasec); }else{$data=$d[fld];}

	if(isset($_GET['page'])){ $noPage = $_GET['page'];} else $noPage = 1;
	$offset = ($noPage - 1) * $d[lmt];

	$query = "SELECT $data FROM $d[tbl] $rest $sortir LIMIT $offset, $d[lmt]  ";
	...
	echo "<form name=myform action=?mod=$d[mod]&menu=aksi method=post ><input type=hidden name=mysubmit >";
	echo "<input type=hidden name=id value=$id >";
	echo "<input type=hidden name=tbl value=$d[tbl] >";
	echo "<input type=hidden name=mod value=$d[mod] >";
	echo "<input type=hidden name=ids >";

 	$result1=mysql_query("SELECT nomor, nama FROM akuntansiv2_akun WHERE tampil='' ");
