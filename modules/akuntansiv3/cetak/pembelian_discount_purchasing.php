...
//PECAH
$column_items='No,Nomor Aju,No Invoice,Nomor Daftar,Tanggal Daftar,Nilai,Debit,Kredit,Nilai USD,Debit USD,Kredit USD,Kode Voucher,Nama Barang,Nama Perusahaan';
...
$query=mysql_query("SELECT * FROM akuntansiv3_jurnal
										WHERE kredit='' AND tanggal BETWEEN '$periode1' AND '$periode2' AND jenis_input='PURCHASING'");
$no =2;
...
while ($rows1=mysql_fetch_array($query)){
	...
	$objPHPExcel->getActiveSheet()->setCellValue("B$no", "$rows1[nomor_aju]");
	$objPHPExcel->getActiveSheet()->setCellValue("C$no", "$rows1[induk_invoice]");
	$objPHPExcel->getActiveSheet()->setCellValue("D$no", "$rows1[nomor_daftar]");
	$objPHPExcel->getActiveSheet()->setCellValue("E$no", "$rows1[tanggal_daftar]");

	$debit=$rows1[debit]-ambil_database("SUM(debit)",akuntansiv3_jurnal,"induk_invoice='$rows1[induk_invoice]' AND kredit='' AND jenis_input IN ('RECEIVE PURCHASING','DISCOUNT PURCHASING')");
	$objPHPExcel->getActiveSheet()->setCellValue("F$no", "$debit");

	$debit_usd=ambil_database("debit",akuntansiv3_jurnal_usd,"induk_invoice='$rows1[induk_invoice]' AND kredit='' AND jenis_input='PURCHASING'")-ambil_database("SUM(debit)",akuntansiv3_jurnal_usd,"nomor_aju='$rows1[nomor_aju]' AND kredit='' AND jenis_input IN ('RECEIVE PURCHASING','DISCOUNT PURCHASING')");
	$objPHPExcel->getActiveSheet()->setCellValue("I$no", "$debit_usd");

	$objPHPExcel->getActiveSheet()->setCellValue("M$no", "$rows1[nama]");
...
header("Content-Disposition: attachment;filename=Discount Purchasing $periode1 s/d $periode2.xls");
