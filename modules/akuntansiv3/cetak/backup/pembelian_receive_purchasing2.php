...
while ($rows1=mysql_fetch_array($query)){
	...
	$debit=$rows1[debit]-ambil_database("SUM(debit)",akuntansiv3_jurnal,"nomor_aju='$rows1[nomor_aju]' AND kredit='' AND jenis_input IN ('RECEIVE PURCHASING','DISCOUNT PURCHASING')");
	$objPHPExcel->getActiveSheet()->setCellValue("F$no", "$debit");

	$debit_usd=ambil_database("debit",akuntansiv3_jurnal_usd,"nomor_aju='$rows1[nomor_aju]' AND kredit='' AND jenis_input='PURCHASING'")-ambil_database("SUM(debit)",akuntansiv3_jurnal_usd,"nomor_aju='$rows1[nomor_aju]' AND kredit='' AND jenis_input IN ('RECEIVE PURCHASING','DISCOUNT PURCHASING')");
