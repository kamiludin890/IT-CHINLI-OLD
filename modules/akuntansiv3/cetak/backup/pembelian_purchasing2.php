...
$query=mysql_query("SELECT inventory_distribusi.nomor_aju,
													 inventory_distribusi.kontak,
 													 inventory_distribusi.nomorinvoiceacc,
 													 inventory_distribusi_items.nomor_doc,
 													 inventory_distribusi_items.tanggal_doc,
 													 inventory_distribusi_items.nilai,
 													 inventory_distribusi_items.nama
										FROM inventory_distribusi
										INNER JOIN inventory_distribusi_items
										ON inventory_distribusi_items.induk=inventory_distribusi.id
										WHERE inventory_distribusi_items.tanggal_doc BETWEEN '$periode1' AND '$periode2' AND inventory_distribusi_items.jenis_doc IN ('BC 2.3 IMPORT','BC 4.0 LOKAL')");
$no =2;
...
while ($rows1=mysql_fetch_array($query)){
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C$no", "$rows1[nomorinvoiceacc]");
	$objPHPExcel->getActiveSheet()->setCellValue("D$no", "$rows1[nomor_doc]");
	$objPHPExcel->getActiveSheet()->setCellValue("E$no", "$rows1[tanggal_doc]");
	$objPHPExcel->getActiveSheet()->setCellValue("F$no", "$rows1[nilai]");
...
header("Content-Disposition: attachment;filename=Purchasing $periode1 s/d $periode2.xls");
