...
function tamount_bayar($nomor_aju,$jenis_doc,$id_posting){
	$result5=mysql_query("SELECT bayar FROM akuntansiv3_posting WHERE nomor_aju='$nomor_aju' AND jenis_doc='$jenis_doc' AND id NOT LIKE '$id_posting'");
...
//}else {
	...
	mysql_query("UPDATE akuntansiv3_posting SET bayar='$bayar[$no]',nilai_ppn='$nilai_ppn[$no]',jumlah_ppn='$hasil_bayar',catatan='$catatan[$no]',ppn='$total_ppn',nominal_debit='$hasil_bayar',nominal_kredit='$hasil_bayar',pencatatan='$pencatatan[$no]' WHERE id='$id_item[$no]'");
//}
...
if ($id_delete) {
	$no=0;for($i=0; $i < count($_POST['id_delete']); ++$i){
		if ($id_delete[$no]){
			mysql_query("DELETE FROM akuntansiv3_posting WHERE id='$id_delete[$no]'");
		}
	$no++;}
}//DELETE END


$column_header='ref,jenis_doc,kontak,tanggal_doc,nomor_aju,invoice_faktur,pencatatan,bayar,nilai_ppn,ppn,jumlah_ppn,catatan';
...
$result=mysql_query("SELECT * FROM akuntansiv3_posting WHERE induk='$id'");
$urut=1;
while ($rows=mysql_fetch_array($result)) {
	...
	echo "<td>$urut</td>";
		$no=0;for($i=0; $i < $nilai_jumlah_pecahan_header; ++$i){
			...
			//pencatatan
			elseif ($pecah_column_header[$no]=='pencatatan') {
				if ($rows[$pecah_column_header[$no]]=='ya'){$hasil='checked';}else{$hasil='';}
				echo "<td><input type='checkbox' name='pencatatan[]' value='ya' $hasil style='width:10px; text-align:right;'></td>";
			}
			//NORMAL
			else{
...
echo "<tr><td colspan='14'><input type='image' src='../../modules/gambar/save.png' width='30' height'30' name='simpan' value='Simpan'></td></tr>";
echo "</form>";
echo "</table>";
//ISI TABEL END
