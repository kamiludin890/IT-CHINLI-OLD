...
if ($jenis_import=='SALES') {
	...
	if ($jenis_import=='RECEIVE SALES' OR $jenis_import=='DISCOUNT SALES') {
					...
					for ($i=2; $i<=$jumlah_baris; $i++){
						...
						$debit_usd=$data->val($i, 8, 0);
						$kredit_usd=$data->val($i, 9, 0);
						$nilai_usd=$data->val($i, 10, 0);

						$receive_date=$data->val($i, 11, 0);
						$kode_voucher=$data->val($i, 12, 0);

						$nama_barang=$data->val($i, 13, 0);
						$perusahaan=$data->val($i, 14, 0);
						$tanggal_transaksi=$data->val($i, 15, 0);
