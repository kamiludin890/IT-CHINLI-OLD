...
//
// 	echo "
// 	<form action='?menu=home&mod=akuntansiv3/bukubesar' method='POST'>
// 	</br>
// 	Bulan
// 	<select name='bulan'>
...
//
// echo "</br>
// <form method ='post' action='?menu=home&mod=akuntansiv3/bukubesar'>
...
if ($list_buku_besar_dokumen==list_buku_besar_dokumen) {
  ...
  $rows4=mysql_fetch_array($result4);
	echo "
			...
			echo "<form method ='post' action='?menu=home&mod=akuntansiv3/bukubesar'>
			<table>
...
$query28="SELECT SUM(bayar) FROM akuntansiv3_posting WHERE kontak='$rows1[kontak]' AND tanggal LIKE '%$rows22[tanggal_pencarian_buku_besar]%' AND jenis_doc IN ('BC 2.7 MASUK','BC 4.0 LOKAL','BC 2.3 IMPORT','BC 2.6.2 HASIL SUBKON')";
$result28=mysql_query($query28);
$row28=mysql_fetch_array($result28);
$total_kredit_posting=$row28['SUM(bayar)'];

$query29="SELECT SUM(bayar) FROM akuntansiv3_posting WHERE kontak='$rows1[kontak]' AND tanggal LIKE '%$rows22[tanggal_pencarian_buku_besar]%' AND jenis_doc IN ('BC 3.0 PEB','BC 2.5 PIB','BC 2.6.1 SUBKON','BC 2.7 KELUAR','BC 4.1 PENJUALAN')";
...
echo "
		...
		<td>KONTAK</td>
		<td>:</td>
		<td><select name='akun' class='comboyuk'>";
		$sql54="SELECT * FROM akuntansiv3_akun WHERE nomor='$_POST[akun]'";
		$result54= mysql_query($sql54);
		$rows54=mysql_fetch_array($result54);
		$nomorakun=$rows54['nomor'];
		$namaakun=$rows54['nama'];

		$sql53="SELECT * FROM akuntansiv3_akun ORDER BY nomor";
		$result53= mysql_query($sql53);
		echo "
...
//Pilihan Akun Awal"
 	//$result1=mysql_query("SELECT nomor, nama FROM akuntansiv3_akun ");
...
if ($akun=='01') {
	...
	$query23="SELECT SUM(debit) FROM akuntansiv3_jurnal WHERE $akun_all AND tanggal BETWEEN '$rows23[saldo_awal_buku_besar1]' AND '$rows23[saldo_awal_buku_besar2]'";
	$result23=mysql_query($query23);
	$row23=mysql_fetch_array($result23);

	$query24="SELECT SUM(kredit) FROM akuntansiv3_jurnal WHERE $akun_all AND tanggal BETWEEN '$rows23[saldo_awal_buku_besar1]' AND '$rows23[saldo_awal_buku_besar2]'";
	$result24=mysql_query($query24);
	$row24=mysql_fetch_array($result24);

	$query31="SELECT * FROM akuntansiv3_jurnal WHERE $akun_all";
	...
	$row31=mysql_fetch_array($result31);
			...
			<table class=table width=100% align=center border=1  style='white-space:nowrap;'>
			...
			<th align=center>Invoice Faktur</th>
			...
			</br><li><a href='modules/akuntansiv3/bukubesar_print.php?akun=$akun&username=admin' target='_blank'>Ekspor To Excel</a></li>
...
$url = "http://localhost/main.php?email=$email_address&event_id=$event_id";
			...
			$jurnal=mysql_query("select akuntansiv3_jurnal.*  from akuntansiv3_jurnal
			where $akun_all and tanggal between '$rows23[mutasi_buku_besar1]' and '$rows23[mutasi_buku_besar2]'
			order by tanggal,id asc") or die (mysql_error());
			$no=1;
			$saldo=$saldoawal;
			...
			$sql214="SELECT invoice_faktur FROM akuntansiv3_posting WHERE ref='$data[ref]'";
			$result214= mysql_query($sql214);
			$rows214=mysql_fetch_array($result214);

			echo "<tr><td align=center >$data[tanggal]</td>
			<td>$data[ref]</td>
			<td>$data[nomor] - ".ambil_database(nama,akuntansiv3_akun,"nomor='$data[nomor]'")."</td>
			<td>$data[kode_posting]</td>
			<td>$rows214[invoice_faktur]</td>
			<td>$data[keterangan_posting]</td>
			...
			echo "
			<tr><th colspan=6 style='text-align:center;'>Jumlah</th>
