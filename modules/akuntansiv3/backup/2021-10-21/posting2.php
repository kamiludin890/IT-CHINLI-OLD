<?php global  $title, $mod, $tbl, $fld ,$p, $akses;
	$mod='akuntansiv3/posting2';
	$tbl='akuntansiv3_posting';
...
// 	echo "</br>
// 	<form method ='post' action='?menu=home&mod=akuntansiv3/posting2'>
...
// 	</br>";
	...
	echo "
	<form action='?menu=home&mod=akuntansiv3/posting2' method='POST'>
	<label>Cari :</label>
	</br>
            ...
            <thead>
              ...
              <th>Catatan</th>
						...
						$rows22=mysql_fetch_array($result22);
									...
									$result = mysql_query("SELECT * FROM inventory_distribusi WHERE $pencarian_data tanggal_doc LIKE '%$rows22[tanggal_pencarian_list_dokumen_posting]%' AND jenis_doc LIKE '%$rows22[dokumen_pencarian_list_dokumen_posting]%'");
									$total = mysql_num_rows($result);
									$pages = ceil($total/$halaman);
									$query = mysql_query("SELECT * FROM inventory_distribusi WHERE $pencarian_data tanggal_doc LIKE '%$rows22[tanggal_pencarian_list_dokumen_posting]%' AND jenis_doc LIKE '%$rows22[dokumen_pencarian_list_dokumen_posting]%' ORDER BY id DESC LIMIT $mulai, $halaman")or die(mysql_error);
									$no =$mulai+1;
									//PAGING
									while ($rows1=mysql_fetch_array($query)){

										$sql21="SELECT * FROM akuntansiv3_posting WHERE nomor_aju='$rows1[nomor_aju]'";
										$result21= mysql_query($sql21);
										$rows21=mysql_fetch_array($result21);

										$query25="SELECT SUM(bayar) FROM akuntansiv3_posting WHERE nomor_aju='$rows1[nomor_aju]'";
										$result25=mysql_query($query25);
										$row25=mysql_fetch_array($result25);

										$totalbayar=$row25['SUM(bayar)'];
										$totalbayar_rupiah=rupiah($totalbayar);
										...
										$nilai_transaksi_rupiah=rupiah($nilai_transaksi);
										...
										// }else {$surat_jalan='';}
																		...
																		echo "<tr>
																						<td>$no</td>
																						<td>$rows1[nomor_aju]</td>
																						<td>$rows1[nomorinvoiceacc]</td>
																						<td>$rows1[nomor_faktur]</td>
																						<td>$rows1[jenis_doc]</td>
																						<td>$rows1[nomor_doc]</td>
																						<td>$rows1[tanggal_doc]</td>
																						<td>$rows1[bukti]</td>
																						<td>$rows1[kontak]</td>
																						<td style='white-space:nowrap; text-align:right;'>$nilai_transaksi_rupiah</td>
																						<td style='white-space:nowrap; text-align:right;'>$totalbayar_rupiah</td>
																						<td style='white-space:nowrap; text-align:right;'>$sisa_rupiah</td>";

																						// $sql223="SELECT status,catatan FROM akuntansiv3_posting WHERE nomor_aju='$rows1[nomor_aju]' AND status='Selesai'";
																						// $result223= mysql_query($sql223);
																						// $rows223=mysql_fetch_array($result223);
																						$catatan_selesai=ambil_database(catatan,akuntansiv3_posting," nomor_aju='$rows1[nomor_aju]' AND status='Selesai'");
																					if(ambil_database(status,akuntansiv3_posting," nomor_aju='$rows1[nomor_aju]' AND status='Selesai'")=='Selesai'){
																						echo "<td>Selesai</td>
																									<td>$catatan_selesai</td>
																					  </tr>";
																					}else{
																							...
																							<input type='hidden' name='nomor_faktur' value='$rows1[nomor_faktur]'/>
																							...
																							<input type='hidden' name='tanggal' value='$rows1[tanggal_doc]'/>
																							<input type='hidden' name='edit_or_tambah' value='tambah'/>
																							<td style='font-weight:bold; text-align:center; '>
																							<input type='image' src='modules/gambar/tambah.png' width='30' height'30' name='simpan' value='Simpan'></td>
																							</form>
																			...
																			<form method ='post' action='?menu=home&mod=akuntansiv3/posting2'>
																				...
																				<input type='hidden' name='kategori_pencarian' value='$kategori_pencarian'>
																				<input type='hidden' name='kolom_pencarian' value='$kolom_pencarian'>
																			...
																			echo "<td> / $pages</td>";
																					 ...
																					 <td><input type='submit' value='".ambil_database(ina,pusat_bahasa,"kode='tampil'")."'></td>
																					</tr>
																					</form>
																					</table>";}
																			//PAGING KLIK END
...
if ($_POST['nomor_faktur']) {
	$invoice_faktur=$_POST['nomor_faktur'];
}else {
  ...
  $nominal_kredit1=$_POST['nominal_kredit'];
	...
	$sql18="SELECT * FROM akuntansiv3_posting WHERE id='$id_posting'";
	...
	$sql6="SELECT * FROM akuntansiv3_persamaan WHERE id='$id_persamaan'";
	...
	$sql9="SELECT * FROM akuntansiv3_akun WHERE nomor='$debit'";
	...
	$sql10="SELECT * FROM akuntansiv3_akun WHERE nomor='$kredit'";
	...
	$sql15="SELECT * FROM akuntansiv3_akun WHERE nomor='$kredit_kedua'";
	...
	function editform($id,$btn){ extract($GLOBALS);
		...
		$sql5="SELECT * FROM akuntansiv3_posting WHERE id='$induk'";
		...
		echo "</br>
					...
					$sql1="SELECT * FROM akuntansiv3_persamaan ORDER BY id";
		...
		<link rel='stylesheet' href='select_picker/css/bootstrap-select.min.css' type='text/css'>
			...
			<table>
			 ...
			 <tr>
				...
				<td>:</td>
				<td><input type='text' class='date' name='tanggal' value='$tgl201'></td>
				<td>Tanggal Input</td>
				<td>:</td>
			 ...
			 <td>Persamaan</td>
			 <td>:</td>
			 <td><select name='persamaan' class='comboyuk' style='width:300px'>";

			 $sql11="SELECT * FROM akuntansiv3_persamaan WHERE id='$rows5[id_persamaan]'";
...
function selesai(){extract($GLOBALS);
	...
	$query="INSERT INTO akuntansiv3_posting SET
	kode= '$nofak'
	,tanggal='$_POST[2]'
	,keterangan='$_POST[3]'
	,persamaan='$_POST[4]'
	...
	$akun=getrow("nomor, nama","akuntansiv3_akun"," where nomor='$_POST[5]'");
	$query="INSERT INTO akuntansiv3_jurnal SET
		ref='$_POST[1]'
		,tanggal='$_POST[2]'
		,nomor='$_POST[5]'
		,nama='$akun[nama]'
	...
	$akun=getrow("nomor, nama","akuntansiv3_akun"," where nomor='$_POST[6]'");
	$query="INSERT INTO akuntansiv3_jurnal SET
		ref='$_POST[1]'
		,tanggal='$_POST[2]'
		,nomor='$_POST[6]'
