<?php global  $title, $mod, $tbl, $fld, $akses ;
	$mod='akuntansiv3/persamaan';
	$tbl='akuntansiv3_persamaan';
...
function editform($id,$btn){ extract($GLOBALS);
	...
	<li><label for='3'>".l(mysql_field_name($row, 3))."</label>". droprow('3','nomor,nama','akuntansiv3_akun',$r[3],' ORDER BY nama ASC')."</li>
	<li><label for='4'>".l(mysql_field_name($row, 4))."</label>". droprow('4','nomor,nama','akuntansiv3_akun',$r[4],' ORDER BY nama ASC')."</li>
...
function doimport(){ extract($GLOBALS);
 ...
 function posting(){extract($GLOBALS);
	...
	$persamaan=getrow("*","akuntansiv3_persamaan"," where id='$checked[$i]'");
	...
	$id=mysql_insert_id();
 	echo "<script type='text/javascript'>window.location.href='?mod=akuntansiv3/posting&menu=persamaan&id=$id'</script>";
//	home();
//	editform($id,'save');
