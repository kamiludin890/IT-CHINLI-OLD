...
if ($nama_database=='aplikasipph_entry' OR $nama_database=='aplikasipph_gaji') {
	...
	if ($_POST['jenis'] == 'edit' AND $ijin_update == 'yes'){
			  $no=0;for($i=0; $i < $nilai_jumlah_pecahan; ++$i){
				...
				if ($nama_database=='aplikasipph_gaji' AND $address=='?mod=aplikasipph/rumus'){
					...
					$no_pensiunan_atau_jht=1;for($i_pensiunan_atau_jht=0; $i_pensiunan_atau_jht < 12; ++$i_pensiunan_atau_jht){$pensiunan_atau_jht=$_POST["gaji$no_pensiunan_atau_jht"]+$pensiunan_atau_jht; $no_pensiunan_atau_jht++;}
					$no_premi_asuransi=1;for($i_premi_asuransi=0; $i_premi_asuransi < 12; ++$i_premi_asuransi){$premi_asuransi=$_POST["premi$no_premi_asuransi"]+$premi_asuransi; $no_premi_asuransi++;}
					$no_iuran_pensiun=1;for($i_iuran_pensiun=0; $i_iuran_pensiun < 12; ++$i_iuran_pensiun){$iuran_pensiun=$_POST["iuran$no_iuran_pensiun"]+$iuran_pensiun; $no_iuran_pensiun++;}
		...
		if ($nama_database=='booking_invoice'){
			...
			$ambil_tahun=substr($tanggal,0,4);
						...
						$nomor=1;for($i=0; $i < $total_dokumen-1; ++$i){//ARRAY TOTAL DOKUMEN
							...
							$no_invoice20="CPMI-$ambil_tahun-$hasil_nol20$ambil_nomor20";
...
<select class='comboyuk' name='$pecah_column[$no]'>
<option value='".$rows5[$pecah_column[$no]]."'>".$rows5[$pecah_column[$no]]."</option>";
echo "<option value='Normal / Aktif'>Normal / Aktif</option>";
echo "<option value='Keluar'>Keluar</option>";
echo "<option value='Meninggal Dunia'>Meninggal Dunia</option>";
echo "
</select>
</td>";
}
...
//PAGING
				...
				elseif ($pecah_column_header[$no_items]=='price_rp' OR
								$address=='?mod=aplikasipph/rumus' AND $pecah_column_header[$no_items]!='tanggal' AND $pecah_column_header[$no_items]!='nama_pegawai' AND $pecah_column_header[$no_items]!='status_pegawai' AND $pecah_column_header[$no_items]!='keterangan_evaluasi' AND $pecah_column_header[$no_items]!='dari_bulan' AND $pecah_column_header[$no_items]!='sampai_bulan' AND $pecah_column_header[$no_items]!='status_ptkp'  AND $pecah_column_header[$no_items]!='pembuat' AND $pecah_column_header[$no_items]!='tgl_dibuat' AND $pecah_column_header[$no_items]!='ket_revisi') {
